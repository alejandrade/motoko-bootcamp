...
actor {
	...
	stable var name_ : Text = "Snowmen MVP";

	stable var symbol_ : Text = "SNOWMEN";
	...
	private stable var principalToTokenIdsEntries : [(Principal, [TokenId])] = [];
	...
	private let principalTokenIds : HashMap.HashMap<Principal, [TokenId]> = HashMap.fromIter<Principal, [TokenId]>(principalToTokenIdsEntries.vals(), 10, Principal.equal, Principal.hash);
	...
	public shared(msg) func mint() : async Nat {
		tokenPk += 1;
		var baseUri = "https://pnxag-qqaaa-aaaam-qacfa-cai.raw.ic0.app/nft/" # Nat.toText(tokenPk) # ".png";
		_mint(msg.caller, tokenPk, baseUri);
        switch (principalTokenIds.get(msg.caller)) {
            case (?opList) {
                var nftList = L.fromArray(opList);
                nftList := L.push(tokenPk, nftList);
                principalTokenIds.put(msg.caller, L.toArray(nftList));
            };
            case null {
                principalTokenIds.put(msg.caller, [tokenPk]);
            };
        };
		return tokenPk;
	};

	public shared(msg) func tokenOfOwnerByIndex(_owner : Principal, _index : Nat) : async ?TokenId {
        switch(principalTokenIds.get(_owner)) {
            case (?v) { return ?v[_index] };
            case null {
                return null;
            };
        };
    ...
    public shared(msg) func tokenOfOwner(_owner : Principal) : async ?[TokenId] {
        switch(principalTokenIds.get(_owner)) {
            case (?v) { return ?v };
            case null {
                return null;
	...
	system func preupgrade() {
		...
		principalToTokenIdsEntries := Iter.toArray(principalTokenIds.entries());
